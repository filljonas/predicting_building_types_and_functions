...
class GCN(torch.nn.Module):
    def __init__(self, input_layer_size, config, output_layer_size):
        ...
        self.conv_layers = nn.ModuleList([GCNConv(i, config.hidden_size, normalize=True)])
        if self.num_gnn_layers > 2:
            self.conv_layers.extend([GCNConv(config.hidden_size, config.hidden_size, normalize=True)
                                     for _ in range(2, self.num_gnn_layers)])
        self.conv_layers.append(GCNConv(config.hidden_size, o, normalize=True))
        if self.fcnn_after:
    ...
    def forward(self, data):
        ...
        distance = (1 - (distance / self.config.minmax_threshold)).view(distance.shape[0])
        ...
        for i, conv_layer in enumerate(self.conv_layers):
            x = conv_layer(x, edge_index, edge_weight=distance)
